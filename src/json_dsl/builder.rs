...
pub struct Builder {
    ...
    schema_builder: Option<Box<dyn Fn(&mut json_schema::builder::Builder) + Send + Sync>>,
...
impl Builder {
    ...
    where
        F: Fn(&OwnedValue, &str) -> validators::ValidatorResult + 'static + Send + Sync,
    {
    ...
    where
        F: Fn(&mut json_schema::builder::Builder) + 'static + Send + Sync,
    {
        self.schema_builder = Some(Box::new(build));
    }

    pub fn build_schemes<V: Value>(
        &mut self,
        scope: &mut json_schema::scope::Scope<V>,
    ) -> Result<(), json_schema::schema::SchemaError> {
    ...
    pub fn process<V: Value>(
        &self,
        val: &mut OwnedValue,
        scope: Option<&json_schema::scope::Scope<V>>,
    ) -> json_schema::validators::ValidationState {
        self.process_nest(val, "", scope)
    }

    pub fn process_nest<V: Value>(
        &self,
        val: &mut OwnedValue,
        path: &str,
        scope: Option<&json_schema::scope::Scope<V>>,
        ...
        let mut state = if val.is_array() {
            let mut state = json_schema::validators::ValidationState::new();

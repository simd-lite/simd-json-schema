...
pub struct ExtendedResult<T> {
    value: T,
    state: json_schema::validators::ValidationState,
}
...
impl<T> ExtendedResult<T> {
    pub fn new(value: T) -> ExtendedResult<T> {
        ExtendedResult {
            value,
            state: json_schema::validators::ValidationState::new(),
        }
    }

    pub fn with_errors(value: T, errors: super::common::error::SimdJsonSchemaErrors) -> ExtendedResult<T> {
        ExtendedResult {
            value,
            state: json_schema::validators::ValidationState {
                errors,
    ...
    pub fn append(&mut self, second: json_schema::validators::ValidationState) {

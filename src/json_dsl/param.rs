...
pub struct Param {
    ...
    pub schema_builder: Option<Box<dyn Fn(&mut json_schema::builder::Builder) + Send + Sync>>,
...
impl Param {
    ...
    where
        F: Fn(&OwnedValue, &str) -> super::validators::ValidatorResult + 'static + Send + Sync,
    {
        self.validators.push(Box::new(validator));
    }

    fn process_validators(&self, val: &OwnedValue, path: &str) -> super::error::SimdJsonSchemaError {
        let mut errors = vec![];
    ...
    ) -> super::ExtendedResult<Option<OwnedValue>> {
